...
class SparseVectorStore(BaseVectorStore):
    ...
    def _calculate_idf(self, doc_count: int, corpus_size: int) -> float:
        ...
        "Your code here"
        idf_score = np.log(corpus_size / (1 + doc_count))
        return idf_score
    ...
    def get_scores(self, query: str):
        ...
        for q in tokenized_query:
            ...
            for doc_id in range(self.corpus_size):
                # Get the document length for the current document
                doc_length = self.doc_len[doc_id]
                # Get the frequency of the token in the current document
                term_frequency = self.doc_freqs[doc_id].get(q, 0)

                # BM25 formula components
                numerator = term_frequency * (self.k1 + 1)
                denominator = term_frequency + self.k1 * (1 - self.b + self.b * (doc_length / self.avgdl))

                # Calculate the current score
                if doc_length > 0:  # Avoid division by zero
                    cur_score = (idf * (numerator / denominator))
                    score[doc_id] += cur_score  # Accumulate score for the document
