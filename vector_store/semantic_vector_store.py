...
class SemanticVectorStore(BaseVectorStore):
    ...
    def _calculate_similarity(
        ...
        "Your code here"
        similarities = np.sort(cos_sim_arr)[::-1][:similarity_top_k]
        node_ids = np.argsort(cos_sim_arr)[::-1][:similarity_top_k]
    ...
    def query(self, query: str, top_k: int = 3) -> VectorStoreQueryResult:
        ...
        if len(doc_embeddings) == 0:
            ...
            result_nodes = [self.node_dict[str(node_id)] for node_id in node_ids]
